...
def ll = new File('prod-item-price-detail.txt').readLines().grep{it.trim()}
def itemLl = []
...
try {
	...
	String sql = """
	insert into t_product_price_detail(${keys}) values(${placeHoders})
	"""

	if(itemLl){
		db.executeUpdate('delete from t_product_price_detail where pid = ?', [itemLl[0].pid])
		for(item in itemLl){
