...
def ll = new File('prod-item-img.txt').readLines().grep{it.trim()}
def itemLl = []
...
try {
	...
	String sql = """
	insert into t_product_img(${keys}) values(${placeHoders})
	"""

	if(itemLl){
		db.executeUpdate('delete from t_product_img where pid = ?', [itemLl[0].pid])
		for(item in itemLl){
