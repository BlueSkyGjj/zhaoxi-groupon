    ...
    <body class="blue">
		...
		<div id="container">
			...
			<div id="content">
				...
				<form method="post" action="{$app_name}/managemailsms/smssend.htm">
				<div style="margin-top: 6px;" style="width: 100%;">
					...
					<div style="float: left; width: 200px; height: 450px; overflow: auto; margin-left: 4px; border: 1px solid silver;">
						<h2 style="font-size: 14px; padding: 2px;">To短信列表</h2>
						<ul id="target-ll" style="padding: 2px;">
						</ul>
					</div>

					<div style="float: left; width: 600px; overflow: auto; margin-left: 4px;">
						<input type="text" style="width: 400px;" name="msg" placeholder="请填写短信内容标题" {if $msg}value="{$msg}"{/if} />
					</div>
				</div>
...
$(function(){
	$('#btn-search').click(function(e){
		...
		$.blockUI({message: '查询中', overlayCSS: {opacity: 0.2}, onBlock: function(){
			$.post(url, {type: 'mobile', target: target, q: q, cate: cate, addr: addr}, function(r){
				if(r.error){
					alert(r.error);
				...
				var tpl = '<li><input type="checkbox" name="l_mobile" value="{0}" checked="checked" />&nbsp;{0}</li>';
				var i = 0;
				for(; i < r.ll.length; i++){
					var one = r.ll[i];
					arr.push(tpl.format(one.mobile));
				}
