 ...
 <div id="content" class="pg-check pg-cart-check"> 
   <div class="mainbox"> 
    <ol class="buy-process-bar"> 
     <li class="step-first"> 1.查看购物车 <span class="highlight"></span> <span class="arrow"></span> </li> 
    ...
    <div class="cart-info-field"> 
     <h4>订单编号-<span style="color: red;">{$orderInfo.orderNo}</span>，项目名称及数量：</h4> 
	...
	{if $noDelivery}
     <ul> 
      <li><span class="delivery-info">手机号码：</span>{$r_addr_mobile}</li> 
      <li><span class="delivery-info">留言：</span>{$r_addr_msg}</li> 
     </ul> 
	{else}
     <h4>物流单的收货地址：</h4> 
     <ul> 
      <li><span class="delivery-info">配送地址：</span>{$delivery.des}</li> 
      <li>
		<span class="delivery-info">配送时间：</span>{$delivery.timeDes}</li>
	  </li> 
     </ul> 
	{/if}
    </div> 
    ...
    <form id="order-check-type-form" class="common-form" action="{$app_name}/orderlist/paypost.htm" method="post" target="_blank"> 
     <input type="hidden" name="order_id" value="{$orderInfo.orderId}" /> 
     <input type="hidden" name="order_no" value="{$orderInfo.orderNo}" /> 
     <div id="J-pay-choice" class="pay-choice"> 
      <p class="pay-choice-tip">应付总额<span class="money">&yen;{$cartSummary.amount}</span></p> 
     ...
     <p class="pay-total" id="J-pay-total">支付<span class="money">&yen;{$cartSummary.amount}</span></p> 
